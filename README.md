...
1. 应用 `migrations/0009_add_perf_job_queue.sql` 至当前最新迁移，部署 Worker，并应用 `wrangler.toml` 中的 `RunnerEventHub` Durable Object migration。
...
启用 `RUNNER_NOTIFICATION_ENABLED=true` 后，Relay 主动建立到 Worker Durable Object 的出站 WebSocket。任务提交后 Worker 只发送 `job_available` 唤醒消息，Relay 仍通过 `/api/runner/jobs/claim` 原子领取任务；WebSocket 断开时每 5 分钟执行一次 HTTPS 对账。关闭开关或缺少 `websocket-client` 时自动恢复原 2 至 30 秒轮询，不要求 Relay 开放任何入站端口。
...
看板默认测试示例以 `docs/perf-examples.json` 和本仓脚本为准。SSH Relay 会在每次默认任务执行前，把所选脚本原子同步到 NPU 主机的 `PERF_REMOTE_WORKDIR`，不依赖服务器上的其他实验仓；以后更新示例脚本时，应提交本仓脚本并同步修改 manifest。只有管理员显式选择已编译部署的源码分支时，测试才使用该分支 `examples/` 下的对应脚本。
...
原始 Prof 由 Relay 回收并压缩，通过 Worker 分片上传到私有 R2 Bucket；登录用户可在看板下载有权限的任务制品。D1 仅保存任务状态、结构化指标和制品元数据，Relay 本地副本默认保留 30 天。R2 将完整对象与活跃上传预留限制在 `9,000,000,000` 字节，达到阈值时按上传日期删除最旧制品并同步清理 D1 记录。R2 上传失败或空间无法释放时结构化结果仍会回传，原始制品退化为仅 Relay 本地保存。
