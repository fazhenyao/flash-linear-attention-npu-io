...
- 4 次用于 16→32 和 32→64 两级 MBH 合并。

MBH 辅助矩阵的大部分区域为零，但原实现仍执行完整 64×64 GEMM。
...
- AscendC kernel 读入后将 `A` 提升为 FP32，MCH 和 MBH 的所有中间矩阵
  及 GEMM 均保持原生 FP32；
- 最终结果转换回输入 dtype；
...
MCH/MBH 中间 BF16 舍入后当前实测到的稳定性成本。
...
累加，但每个 MCH/MBH 基块结果按旧实现落回 BF16，再作为下一阶段输入。
...
最后一个等号利用了 16×16 严格下三角矩阵必有 `A¹⁶=0`。这里的“必有”
是矩阵结构带来的：一次乘法至少把依赖向下推进一行，推进 16 次后已经
越过整个 16×16 基块，并不是假设 `A` 的元素很小。

#### 8.2.2 MBH 公式

MCH 分别求出 16×16 对角基块后，MBH 按块下三角矩阵的逆公式合并。
设待合并的 `2b×2b` 块为：
...
也就是说，两个已经求好的对角逆块 `R₀/R₁` 原样保留，只需用两次矩阵乘
得到新的左下块：
...
MCH/MBH 阶段后把结果转回 BF16，再送进下一次 GEMM。BF16 舍入误差因此
不是只发生一次，而是被下一次平方、乘法和块合并继续放大。
...
| 16→32 MBH | 5.3125 | 1 | 5.3125 | 1.7245898 |
| 32→64 MBH / 最终 | 192 | 1 | 192 | 43.484668 |

#### 8.2.3 为什么 `A⁸` 到 MCH 第三轮出现相对误差阶跃
...
因此不能只看 `A⁸` 自身约 1.16% 的相对 L2 误差，还要看加法两侧是否
发生相消。在上述构造的 `[13, 0]` 元素上，逐式复现得到：

| 第三轮 `[13,0]` | FP32 中间路径 | 旧 BF16 阶段存储路径 |
| --- | ---: | ---: |
| `X₂` | -82.660995 | -83 |
| `(X₂A⁸)` | +82.661011 | +84.218857 |
| 相加后、最终存储前 | 约 `1.5259e-5` | 约 `1.218857` |
| `X₃` 存储值 | 约 `1.5259e-5` | 1.21875 |

右列的乘积仍以 FP32 累加，但乘法输入已经是前几轮落盘后的 BF16 值；
因此它只能准确计算“已经被舍入的输入”的乘积，不能恢复此前丢失的位。
对这个重复 key 构造，数学上该位置的逆矩阵元素是
`-beta·(1-beta)¹² ≈ -3.93e-9`。FP32 递推也会因有限精度留下约
`1.53e-5` 的残差，但 BF16 路径留下的是 `1.21875`，两者已经相差约
八万倍。

在 FP32 路径中，加法两侧绝对值之和约为 165.322，而相加结果只有约
`1.53e-5`；相消比例约为 `1.08e7`，相当于丢掉约 23.4 个二进制有效位。
BF16 总共只有 8 个二进制有效位，无法在约 83 的两个数相消时保留
`1e-5` 量级残差；前两轮对 `X₂`、`A⁸` 以及点积各项的舍入，最终在该
位置形成约 1.22 的绝对误差。

这也解释了表中看起来突兀的相对 L2 阶跃：`A⁸` 的 1.16% 是相消前的
整体误差；MCH 第三轮的相对 L2 是用相消后的正确 `X₃` 作分母。相消使
正确结果的非对角元素显著缩小，但已有绝对误差并不会同步缩小，于是相对
L2 从 0.0116 跳到 0.4547。这不是 BF16 指数上溢，而是“分母因相消变小，
舍入残差仍然很大”。

MBH 随后计算 `-R₁A₁₀R₀`，会把错误的基块逆矩阵同时放在乘法左右两侧。
因此 MCH 留下的伪残差又经过 16→32、32→64 两级乘法进入更大的左下块，
相对 L2 继续增至 1.7246 和 43.4847，最终把小的舍入偏差变成数量级错误。
...
- 接口输入和最终输出为 BF16，MCH/MBH 中间计算为原生 FP32。
...
- 当前未给任意矩阵或 `BT≠64` 的完整链给出统一输出上限；如果扩展这些
  场景，应重新检查 SolveTri 残差、MCH/MBH 中间最大值和逐 chunk
  误差放大倍数。
...
- FP32 修复消除了每轮 MCH/MBH 的 BF16 舍入，仅保留适配层把 KKT 的
