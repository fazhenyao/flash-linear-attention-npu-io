...
性能测试由 Relay 启动独立远端 supervisor。每次 job + attempt 使用独立控制目录和随机进程标记；Python、profiler 及继承标记的子进程均属于本次尝试。

看板取消请求写入队列后，通过现有 WebSocket 发送 `job_cancel_requested`（携带 `job_id`、`attempt_id`）；任务心跳保留为通知丢失或断连时的兜底。Relay 通过另一条 SSH 连接发起取消：先发送 SIGTERM，3 秒后仍存活则发送 SIGKILL。终止前核对 Linux PID 的启动时间，扫描包含独立会话在内的本次任务子进程，确认退出后才上报已取消。未确认退出时持续重试，不能把 SSH 断开当成取消成功。

取消标记与启动由同一文件锁协调：取消先到则不启动命令。控制目录保留取消标记，避免迟到启动；完成/取消接口保护最终状态，旧 attempt 通知不作用于重试任务。设备内核退出和资源释放可能有延迟，处于内核不可中断等待的进程可能无法立即结束；该机制不会重置设备或终止其他任务。

启用需要部署 Worker 并重启 Relay；新启动的 SSH 性能任务才受 supervisor 管理。升级前已经运行的任务没有进程标记，不能套用此机制自动终止。现有持久编译任务沿用编译取消实现。
